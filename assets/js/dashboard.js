...
function initDashboardFromRawCSV(raw){
  ...
  const cuartelDropdown = document.querySelector("#f_cuartel");
  if(cuartelDropdown){
    const cuartelBtn = cuartelDropdown.querySelector('.mes-dropdown-btn');
    if(cuartelBtn && cuartelBtn.dataset.bound !== '1'){
      cuartelBtn.dataset.bound = '1';
      cuartelBtn.addEventListener('click', e=>{
        e.stopPropagation();
        const willOpen = !cuartelDropdown.classList.contains('open');
        cuartelDropdown.classList.toggle('open', willOpen);
        cuartelBtn.setAttribute('aria-expanded', willOpen ? 'true' : 'false');
      });
    }
    if(cuartelDropdown.dataset.bound !== '1'){
      cuartelDropdown.dataset.bound = '1';
      cuartelDropdown.addEventListener('input', e=>{
        if(e.target.id === 'cuartel_search') populateCombos();
      });
      cuartelDropdown.addEventListener('change', e=>{
        const checkbox = e.target;
        if(!checkbox.matches('input[type="checkbox"]')) return;
        const panel = cuartelDropdown.querySelector('.mes-dropdown-panel');
        const todosCheck = panel.querySelector('#cuartel_todos');
        const cuChecks = Array.from(panel.querySelectorAll('input[data-cuartel]'));
        if(checkbox.id === 'cuartel_todos'){
          if(checkbox.checked){
            cuChecks.forEach(c=>c.checked = true);
            state.filtros.cuartel = 'Todos';
          }else{
            cuChecks.forEach(c=>c.checked = false);
            state.filtros.cuartel = [];
          }
        }else{
          const checked = cuChecks.filter(c=>c.checked).map(c=>c.dataset.cuartel);
          if(checked.length===cuChecks.length){
            state.filtros.cuartel = 'Todos';
    ...
    if(document.documentElement.dataset.cuartelDocBound !== '1'){
      document.documentElement.dataset.cuartelDocBound = '1';
      document.addEventListener('click', e=>{
        if(!cuartelDropdown.contains(e.target)){
          cuartelDropdown.classList.remove('open');
          if(cuartelBtn) cuartelBtn.setAttribute('aria-expanded','false');
        }
      });
    }
  }
